...
class DepthwiseConv(nn.Module):
    def __init__(self, in_channels, out_channels, expanded = 6):
        super().__init__()
        expanded = in_channels * expanded
        # depthwise  
        self.pointwise1 = nn.Conv2d(in_channels, expanded, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(expanded)
        self.relu1 = nn.ReLU6(inplace=True)
        
        self.depthwise = nn.Conv2d(expanded, expanded, kernel_size=3, padding=1, groups=expanded, bias=False)
        self.bn2 = nn.BatchNorm2d(expanded)
        self.relu2 = nn.ReLU6(inplace=True)

        self.pointwise2 = nn.Conv2d(expanded, out_channels, kernel_size=1, bias=False)
