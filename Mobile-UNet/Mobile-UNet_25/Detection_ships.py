#!/usr/bin/env python3
"""
trainModelVessel_v2.py
----------------------
Entrenamiento de UNet para segmentacion de embarcaciones aereas.

Esta version consume directamente los tiles generados por build_tiles.py:
...
Diferencias respecto al script "adaptado de incendios":
  - Batching real (el original hacia un optimizer.step() por muestra).
  - Rutas configurables (--data-root o $BASE_DIR).
  - Lee tiles ya tileados a resolucion nativa, sin cv2.resize.
  - Metricas por matriz de confusion acumulada, no promediadas por imagen.
  - Pesos de clase calculados del manifiesto.
  - Clamp defensivo de mascaras.
  - Checkpoint reanudable.
  - Manejo de OOM en GPU con sugerencias.

Uso:
    export BASE_DIR=/home/liese2/SPRI_AI_project
    python trainModelVessel_v2.py --epochs 60 -b 16 --lr 1e-3 --gpu auto
"""
...
# ---------------------------------------------------------------------------
# Modelo: usa Vessel_models si esta disponible; si no, Wildfire_models como
# fallback. La interfaz UNet2D es identica en ambos.
# ---------------------------------------------------------------------------
try:
    from Vessel_models import UNet2D as VesselNet
    MODEL_BACKEND = "Vessel_models"
except ImportError:
    try:
        from Wildfire_models import UNet2D as VesselNet
        MODEL_BACKEND = "Wildfire_models"
    except ImportError:
        print("ERROR: no encuentro ni Vessel_models.UNet2D ni Wildfire_models.UNet2D")
        sys.exit(1)
...
class VesselTileDataset(Dataset):
    """
    Lee tiles ya recortados a resolucion nativa. No reescala.
    Los nombres vienen del <split>.txt (sin extension).
    """
...
class Augmentation:
    """Flips, rot90, brillo y ruido. Validas para vista cenital."""
...
class ConfusionMatrix:
    """Matriz de confusion acumulada en GPU. De aqui sale IoU por clase correcto."""
...
def parse_args():
    p = argparse.ArgumentParser(description="Vessel Segmentation Training")
    ...
    p.add_argument("--gpu", default="auto", type=str)
    ...
    p.add_argument("--dice-weight", default=0.5, type=float)
    ...
    p.add_argument("--subset", default=0, type=int)
...
def main():
    ...
    print("=" * 70)
    print(f"VERIFICANDO SISTEMA (backend de modelo: {MODEL_BACKEND})")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        try:
            ...
            if not args.grad_checkpoint:
                print(f"  1. --grad-checkpoint")
                print(f"  2. -b {max(1, args.batch_size // 2)}")
            else:
                print(f"  1. -b {max(1, args.batch_size // 2)}")
                print(f"  2. --base-channels 16")
            print("  3. PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True")
            print("=" * 70)
            log.close()
            sys.exit(1)
